...
This project follows the **Baton** rule-set (see `~/.claude/baton/` for full rule docs and provenance). Twelve rules, listed in priority order:
...
- **Test results** — output of the slice-relevant test commands (the tests that directly cover the slice's scope); do NOT run the full suite here — the merge gate owns full-suite verification
...
Full rule docs with provenance and detailed examples: `~/.claude/baton/`. Release Mode harness (slice template + role prompts + first-pass script): `~/.claude/baton/release-mode-template/`, `~/.claude/baton/role-prompts/`, `~/.claude/bin/release-verify.sh`.
