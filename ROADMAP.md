...
- **LLM check types (6)** — `spec-ambiguity`, `design-review`, `ac-satisfaction`, `security-review`, `semantic-coverage`, `maintainability-review`. Deterministic (temp=0), structured prompts, structured JSON output, fail-closed. The prompt bodies are specified in `baton/llm-checks/`; spec ambiguity returns `spec-ambiguity-report-v1`, while the other five return `llm-check-report-v1`. Conformant engines expose each stable check id through adapter-owned invocation syntax.
...
- **JSON Schemas** — record schemas validating the emitted loop records (`board-v1`, `spec-v1`, `proof-v1`, `slice-status-v1`, `journeys-v1`, `attestations-v1`, `verifier-verdict-v1`, `contracts-v1`, `assembly-proof-v1`, `llm-check-report-v1`, `spec-ambiguity-report-v1`), plus config/policy schemas (`architecture-rules-v1`, `design-fidelity-v1`, `design-allowlist-v1`, `architecture-overrides-v1`, `capability-policy-v1`, `project-context-v1`). Hosted at `baton.sawy3r.net/schemas/`.
