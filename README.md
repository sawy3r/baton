...
```json
{
  "slice_id": "S03-portfolio-add-flow",
  "state": "implemented",
  "planned_files": ["apps/web/src/portfolio/PortfolioDrawer.tsx", "e2e/portfolio-add.spec.ts"],
  "actual_files": ["apps/web/src/portfolio/PortfolioDrawer.tsx", "apps/web/src/portfolio/usePortfolioStore.ts", "e2e/portfolio-add.spec.ts"],
  "test_commands": ["pnpm playwright test e2e/portfolio-add.spec.ts"],
  "reachability_artifacts": ["e2e/portfolio-add.spec.ts:24 — user gesture + assertion"],
...
```markdown
# proof — S03-portfolio-add-flow

## Scope
User can add a holding via the portfolio drawer and see it in their projection.

## Files changed
3 files; e2e/portfolio-add.spec.ts is new and exercises the integration point.

## Test results
Playwright suite — 8 tests, all green. Captured output: ...

## Reachability artefact
e2e/portfolio-add.spec.ts:24 simulates the full user gesture
(click → form fill → submit) and asserts the new row renders.

## Delivered
- Form submit creates row in store
- Projection re-renders within 200ms

## Not delivered
- Multi-currency holdings (deferred to S05; tracked in journal.md, acknowledged 2026-05-12)
```
...
1. A `docs/release/` directory at the repo root (the commands create sub-folders here per release). If your docs site renders content from a different location (e.g. Fumadocs at `apps/docs/content/docs/`), symlink it: `ln -s apps/docs/content/docs docs`.
