...
2. **Enforce sequential order within the track.** For every slice listed *before* `$1` in the track's `.slices`, read its complete status with `git show <worktree_branch>:docs/release/$2/<earlier>/status.json`. Accept `verified` / `shipped` and a legal unstarted Rule-2 deferral. Accept a `deferred` terminal `re_slice_required` or `protocol_history_invalid` original when `$1` is its recorded rollback; for any other later slice, require that rollback already to be `verified` / `shipped`, ordered before `$1`, and backed by the applicable passing tree-equality proof. A protocol-history retirement also requires every invalid-history record at the owner's exact status path with matching parsed slice/release identity and strict first-parent presence, plus a duplicate-free, historical-schema-valid fresh Verifier BLOCKED status strictly before retirement. Require byte-identical `passed` maintainability with a concrete head and qualifying PASS ledger. Retirement, rollback first status, qualifying rollback verdict, and replacement `start_commit` must be distinct in that strict first-parent order; ordinary ancestry is insufficient. Otherwise BLOCK: "Slice `<earlier>` precedes `$1` in track `<track-id>` (state `<state>`) and is not integration-ready. Finish its verification or mandatory rollback first."
