...
1. **Discover the slice's track via the board oracle** (reference implementation: `sworn board --json`). This discovery step runs *before* any worktree is known, so the worktree-anchor guards used everywhere else in Step 0 do not yet apply — but the oracle needs no anchor: it reads the board (`board.json`) and every `status.json` straight from the `release-wt/$2` and `track/$2/*` **git refs**, which resolve from any cwd on any branch. The integration branch the launch directory sits on receives the board **only at `/merge-release` time**, so a by-hand launch-directory read would silently miss every slice and track a `/replan-release` added — the oracle is immune to that. Run:
   ...
   Two distinct failures, two distinct remedies — do not conflate them. If the oracle command is **not on PATH**, return `BLOCKED: no Baton engine installed — Release Mode requires a conformant engine (reference implementation: go install github.com/swornagent/sworn/cmd/sworn@latest).` If the oracle **is installed but exits non-zero**, it ran and could not resolve the board: return `BLOCKED: board oracle failed: <the engine's stderr, verbatim>` — do NOT advise installing or repairing the engine, and do not paraphrase its error. Parse `.releases["$2"]` and find the `.tracks[]` entry for which `any(.slices[]; .id == "$1")` is true. If `$1` is in no track, return `BLOCKED: slice '$1' is not assigned to a track in board.json.` (The oracle resolves track membership from `release-wt/$2`; this cannot be a stale-launch-directory false negative.)
2. From that track entry capture `<track-id>` (`.id`). Derive
   `<worktree_branch>` as `track/<release-name>/<track-id>` and
   `<worktree_path>` as
   `$HOME/projects/<REPO_BASENAME>-worktrees/release-<release-name>-<track-id>`.
   Neither value is a required board-oracle field. The disk check in step 3 is
   the authoritative existence test even when optional oracle convenience
   metadata is absent or disagrees.
3. Run `git worktree list --porcelain` and confirm the exact `<worktree_path>` / `refs/heads/<worktree_branch>` stanza exists. If absent, return `BLOCKED: track '<track-id>' worktree is missing on disk. Verification requires that implementation happened in a track worktree — have the implementer run /implement-slice for a slice in this track first, or recreate it with 'git worktree add <worktree_path> <worktree_branch>'.`
