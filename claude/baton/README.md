...
- **Gate scripts (7)** — `release-trace.sh` (RTM + EARS + sniff-test), `release-coverage.sh` (AC → test mapping), `release-audit-design.sh` (colours + architecture rules), `release-mock-check.sh` (undeclared mock boundaries), `release-regression.sh` (post-merge full suite), `release-verify.sh` (proof bundle structure), `release-board-status.sh` (state machine verifier).
