...
Slice S06-checkout-view-playwright authored 5 Playwright walks against `/checkout`. The anonymous walk PASSed. The four pro-tier walks 500'd at SSR with `ReferenceError: resolvedTier is not defined at DashboardLayout.tsx:412`. `git blame` showed "Not Committed Yet" — peer slice S07-rule-editor-surface had `DashboardLayout.tsx` in its uncommitted WIP, with `resolvedTier` renamed to `resolvedUserTier` everywhere *except* line 412.

`DashboardLayout.tsx` was not in S06's planned files. It was reachable from `/checkout` SSR for authenticated users.
...
for route in $slice_routes_tested; do
  ssr_reachable=$(find src/app/$route/ src/components/ -name "*.tsx" -o -name "*.ts")
...
There is no merge conflict, no error message. A read of `docs/release/<sibling-release>/<slice>/status.json` from inside the worktree returns the state at fork time, not the current state on integration.

### Concrete case

A briefer (running in the primary repo on `release/v0.5.0`) told an implementer (on a `release-wt/2026-05-17-checkout-view` worktree) that account-settings slice S05b was `verified` and its routing was wired. The implementer ran a five-second audit:

- The worktree's copy of `account-settings/S05b-settings-target/status.json` read `state: planned`.
- The worktree's `src/lib/billing/types.ts` at line 412 had only the constant declaration, no routing.
...
```bash
grep '"state"' /path/to/<implementer-worktree>/docs/release/<sibling>/<slice>/status.json
```
