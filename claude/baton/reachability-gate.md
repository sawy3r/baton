...
The v0.5.0 audit on GetFired's monorepo (May 2026) found five primitives shipped as dark code, all with passing TDD-written unit tests: per-section Summary/Detailed mode toggle (component prop existed, parent hardcoded the literal), `SectionStatusDot` (built + tested, zero render sites), `FieldIssueIndicator` (built, no consumers), `useScenarioWorkflow` (496 lines, 26 tests, no consumer), per-line `inflationRate` Premium gate inside `ExpensesSection.Detailed` branch (Detailed mode unreachable from UI). Each had been "done" at the leaf-component test level. None were reachable.
