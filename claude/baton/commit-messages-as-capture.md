...
```
feat(workspace): add Summary/Detailed toggle (see plan 2026-05-11)
```

### Strong

```
feat(workspace): per-section Summary/Detailed toggle with progressive disclosure

Each workspace section (Identity & Household, Income & Super, Expenses,
Property & Debt, Investing) now carries its own Summary/Detailed toggle
rather than a global mode. Default is Summary; users open Detailed only
on the sections that need it.

Rationale: a global toggle either hides too much (Summary global) or
overwhelms newcomers (Detailed global). Per-section progressive
...
The strong version survives any future move of the plan doc. It also surfaces in `git log` searches for "progressive disclosure" or "Summary/Detailed" without depending on external state.
