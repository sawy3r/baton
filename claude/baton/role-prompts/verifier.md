...
1. **Read the release board from the `release-wt/<release-name>` branch — never the launch-directory working copy.** The board (`index.md`) is maintained on `release-wt/<release-name>` and the track branches; the integration branch the launch directory sits on receives it only at `/merge-release` time, so a launch-directory read silently misses every slice and track `/replan-release` added after the release was cut. This is the discovery step — it runs before any worktree is known, so the worktree-anchor guards used elsewhere in Step 0 do not yet apply; the branch ref is the only anchor available. Read it with `git show release-wt/<release-name>:docs/release/<release-name>/index.md`. In that board's `tracks:` list, find the track whose `slices` array contains `<slice-id>`. If `<slice-id>` is in no track — **first re-confirm you read the `release-wt` copy via `git show`, not the launch directory** — `BLOCKED: slice '<slice-id>' is not assigned to a track in index.md.`
...
5. **Drift gate — forward-merge `release-wt/<release-name>` into the track worktree.** Before reading any artefact, sync the track to the release assembly branch — the same self-healing merge `/implement-slice` Step 0 and `/merge-track` Step 0 run. A `/replan-release` re-scope commits the corrected `spec.md` to `release-wt/<release-name>`; it reaches the track branch *only* via this merge. A verifier that reads `spec.md` without this step reads a **stale** spec, re-derives the same BLOCKED, and the slice re-enters an unbreakable `/verify-slice` ↔ `/replan-release` loop. `/verify-slice` was historically the lone track-worktree command that read track artefacts without first forward-merging `release-wt`; this gate removes that asymmetry.
   ...
   - Otherwise `git -C <worktree_path> merge release-wt/<release-name> --no-edit`. By track-mode invariant 2 the in-flight `release-wt` delta is touchpoint-disjoint from this track → conflict-free on code; a docs-only merge (`spec.md`, `index.md`) is expected and proceeds silently.
   - A **code or test** conflict (`git -C <worktree_path> diff --name-only --diff-filter=U`) is a real touchpoint-matrix error (invariant 4): `git -C <worktree_path> merge --abort` and `BLOCKED: forward-merge of release-wt/<release-name> into the track conflicted on <files> — route to /replan-release to re-group.` A docs-only conflict (`index.md`) you resolve to the union of both sides and continue. Push the synced track branch so the merge is durable.
