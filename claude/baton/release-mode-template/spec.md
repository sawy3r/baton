...
<One sentence. Names a user, a gesture, and an observable result. Example: "A premium user clicks Export PDF on the calculator and receives a downloaded PDF; a free user clicks the same control and is shown the upgrade modal.">

## Entry point

<The route, page, API endpoint, or user gesture that triggers the outcome. Must be a concrete user-reachable surface. Example: "Button on /calculator page with data-testid='control-panel-export-pdf'; backend endpoint POST /api/fire/export-pdf">
...
<Concrete, named risks for this slice. Not generic. Example: "Stripe webhook race condition between checkout completion and entitlement write — verified by ordering test in TestStripeWebhookOrdering">
