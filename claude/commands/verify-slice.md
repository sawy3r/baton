...
1. **Discover the slice's track via the board oracle.** This discovery step runs *before* any worktree is known, so the worktree-anchor guards used everywhere else in Step 0 do not yet apply — but the oracle needs no anchor: it reads the board (`index.md`) and every `status.json` straight from the `release-wt/$2` and `track/$2/*` **git refs**, which resolve from any cwd on any branch. The integration branch the launch directory sits on receives the board **only at `/merge-release` time**, so a by-hand launch-directory read would silently miss every slice and track a `/replan-release` added — the oracle is immune to that. Run:

   ```
   $HOME/.claude/bin/release-board-status.sh --json
   ```

   If the script is missing or exits non-zero, return `BLOCKED: release board oracle unavailable — install baton (~/.claude/bin/) before verifying.` Parse `.releases["$2"]` and, in `.tracks[]`, find the entry whose `.slices` array contains `$1`. If `$1` is in no track, return `BLOCKED: slice '$1' is not assigned to a track in index.md.` (The oracle resolves track membership from `release-wt/$2`; this cannot be a stale-launch-directory false negative.)
