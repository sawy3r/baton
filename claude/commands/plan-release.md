...
2. Check whether `docs/release/$1/` already exists.
   ...
   - If it does exist, first check whether a `release-wt/$1` branch exists (`git rev-parse --verify release-wt/$1`). **If it does, the release is already in flight** — STOP and tell the human to use `/replan-release $1` instead; `/plan-release` is for pre-flight planning only, and the board's one home is now `release-wt/$1` (track-mode.md invariant 5). If there is no `release-wt/$1` branch, the release is still mid-planning: read `intake.md` and `index.md` in full before responding — you are continuing, not starting. State the current slice count and any slices not yet at `verified` state in your first message back.
