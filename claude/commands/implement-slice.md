...
1. **Find the slice's track — read the board from `release-wt/$2`, never the launch directory.** The release board `index.md` has one home: the `release-wt/$2` branch (track-mode.md invariant 5). The launch directory sits on the integration branch, which receives the board only at `/merge-release` time — a launch-directory read silently misses every slice and track `/replan-release` added after the release was cut, and yields a spurious `BLOCKED: slice not assigned to a track`. This is the discovery step itself; it runs before any worktree is known, so the worktree-anchor guards used elsewhere in Step 0 do not yet apply — the branch ref is the only available anchor. Read the canonical board with `git show release-wt/$2:docs/release/$2/index.md` (the `release-wt/$2` ref lives in the shared object store, so this resolves from any cwd). If `release-wt/$2` does not exist, no `/implement-slice` has run for this release yet — the release worktree has not been created and the launch-directory copy `docs/release/$2/index.md` is the current seed; read that instead. In the board's `tracks:` list, find the entry whose `slices` array contains `$1`. If `$1` is in no track — **first re-confirm you read the `release-wt/$2` copy via `git show`, not the launch directory** (a launch-directory read is the most common cause of a false negative here) — then BLOCK: "Slice `$1` is not assigned to a track in `index.md` — re-run `/plan-release $2` (or `/replan-release $2`) to group it." Capture `<track-id>`, `worktree_path`, `worktree_branch`, `depends_on`, and the ordered `slices`.
2. **Enforce sequential order within the track.** This gate reads each earlier slice's `status.json` — and it MUST read from the **track worktree**, not the launch directory. The launch directory is the primary repo on the integration branch, which does *not* carry the track's implementer/verifier commits: a slice that is `verified` on the track branch still reads `planned` there. An unanchored read here is the documented cause of an `/implement-slice` ↔ BLOCK loop — `/implement-slice S03` reads stale `planned` and blocks "do S01 first"; `/implement-slice S01` anchors correctly, sees S01 already `verified`, and bounces back to S03.
   - If the track's `worktree_path` (captured in step 1) is **unset**, no track worktree exists yet, so no slice in the track has been implemented — the gate is trivially satisfied; skip to step 4.
   - Otherwise, for every slice listed *before* `$1` in this track's ordered `slices`, read `<worktree_path>/docs/release/$2/<earlier>/status.json` and take its `state`. If any is not `verified`, BLOCK: "Slice `<earlier>` precedes `$1` in track `<track-id>` (state `<state>`). Slices in a track are implemented in order — finish and verify `<earlier>` first."
3. **If the track's `worktree_path` is set** (track worktree already materialised):
   ...
   - Capture `<worktree_path>`. **For the rest of this session, every Bash command runs `cd <worktree_path> && <cmd>` (or `git -C <worktree_path>`); every Read/Write/Edit uses an absolute path anchored at `<worktree_path>`.** Skip to step 5 (the BLOCKED-verdict guard).
4. **If the track's `worktree_path` is NOT set** (first `/implement-slice` for this track): materialise it.
   - **Release worktree first.** If `release_worktree_path` is unset in frontmatter, this is also the first `/implement-slice` in the release: parse the integration branch from `index.md` "Release summary" → `Target version / integration branch` (e.g. `release/v0.5.0`), then `git worktree add $HOME/projects/<REPO_BASENAME>-worktrees/release-$2 -b release-wt/$2 <integration-branch>`. You record `release_worktree_path` + `release_worktree_branch` in the board in the final sub-step below — every board write for this command happens once, together, on `release-wt/$2`.
   ...
   - **Update the board on `release-wt/$2`, never the integration branch** (track-mode.md invariant 5). Edit `<release_worktree_path>/docs/release/$2/index.md` frontmatter — set this track's `worktree_path` and `state: in_progress`, plus `release_worktree_path` + `release_worktree_branch` if this was the release's first slice. Commit on `release-wt/$2` from inside the release worktree, then push it: `git -C <release_worktree_path> commit ... -m "chore(release/$2): materialise worktree for track <track-id>"` followed by `git -C <release_worktree_path> push origin HEAD:refs/heads/release-wt/$2`. Do **not** write `index.md` in the launch directory and do **not** commit on the integration branch — that produces the partial-frankenstein board (worktree paths but no `/replan-release` slices) that invariant 5 exists to prevent.
   - Treat the new worktree as `<worktree_path>` per step 3. Continue silently — no human handoff.

5. **BLOCKED-verdict guard.** With `<worktree_path>` captured, read the target slice's own `status.json` from the **track worktree** — `<worktree_path>/docs/release/$2/$1/status.json`, never the launch-directory copy (the same anchoring rule as step 2). If its `verification.result` is `"blocked"`, BLOCK immediately and do not start implementation:
