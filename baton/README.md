...
- **LLM check types (6)** — `spec-ambiguity` (planner), `design-review` (captain), `ac-satisfaction` (implementer + verifier), `security-review` (implementer + verifier), `semantic-coverage` (verifier), `maintainability-review` (Implementer readiness preflight + authoritative fresh Verifier gate). Deterministic (temp=0), structured prompts, structured JSON output, fail-closed. Engines expose these operations by check id; CLI spelling is adapter-owned and non-normative. **The prompt bodies are specified** in [`llm-checks/`](llm-checks/) — the prompt IS the contract, the same way a schema is, so an engine cannot be conformant without them. Spec ambiguity returns `spec-ambiguity-report-v1`; the other five return `llm-check-report-v1`.
...
- **JSON Schemas** — record schemas validating the emitted loop records (`board-v1`, `spec-v1`, `proof-v1`, `slice-status-v1`, `journeys-v1`, `attestations-v1`, `llm-check-report-v1`, `spec-ambiguity-report-v1`), the public board read model (`board-oracle-v1`), plus config/declaration schemas (`architecture-rules-v1`, `design-fidelity-v1`, `design-allowlist-v1`, `architecture-overrides-v1`, `project-context-v1`). Hosted at `baton.sawy3r.net/schemas/`.
