...
Duplicate-aware parse and schema-validate current predecessor status before inspecting retirement fields; any schema error BLOCKs immediately. Compare the first retirement commit's raw retirement JSON value bytes with every later owner first-parent status version through the evaluated ref; removal, nulling, any rewrite and mutate-then-restore BLOCK.
