...
4. **Track worktree absent** from `git worktree list` (first `/implement-slice` for this track), materialise it — **writing nothing to `release-wt`**:
   a. **Ensure the release worktree exists.** If the release worktree (conventional path `$HOME/projects/<REPO_BASENAME>-worktrees/release-<release-name>`, branch `release-wt/<release-name>`) is absent from `git worktree list`, this is also the first `/implement-slice` in the release: read `release.integration_branch` from `board.json`, then `git worktree add $HOME/projects/<REPO_BASENAME>-worktrees/release-<release-name> -b release-wt/<release-name> <integration-branch>`. This creates the `release-wt` branch; it does **not** commit to it, and nothing is written to the board.
