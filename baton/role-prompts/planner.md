...
- **Drop or re-scope a started slice** → a human decision surfaced explicitly; `in_progress` / `verified` / `merged` work is never silently rewritten. When re-slicing resolves `maintainability.state: re_slice_required`, reset the status object's `maintainability` field to the template's `pending` cycle-0 record as part of the ratified spec change. A materially different spec for an already-`verified` slice is a **new slice** (new id), not an edit — verified work is immutable.
