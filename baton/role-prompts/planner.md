...
2. A release **board record** at `docs/release/<release-name>/board.json`, valid against the `board-v1` schema (`https://baton.sawy3r.net/schemas/board-v1.json`): every track, its ordered slice-id list, `depends_on`, and the release `vertical_trace`. `board.json` is a **pure plan** — a track's worktree path and its `planned`/`in_progress`/`merged` state are derived from git refs by the oracle (invariant 5), never stored. The human-readable `index.md` (tracks table, touchpoint matrix, dependency diagram) is **rendered from** `board.json` — never hand-maintained in parallel.
...
3. **Build the contract registry.** The touchpoint matrix proves file-level disjointness; the registry proves contract-level coherence. Emit `docs/release/<release-name>/contracts.json`, valid against `contracts-v1` (`https://baton.sawy3r.net/schemas/contracts-v1.json`): every wire artefact surfaced by Layer 4's wire-surface enumeration (endpoint, header, cookie, env var, edge config, schema version, db schema, storage key, feature flag, auth scope, event) becomes an entry with **exactly one owning slice**, its `consumers`, the owner's `live_test` (required whenever consumers exist), and — for anything crossing HTTP middleware — an `edge_config` disposition: the sibling `edge-config` entry that owns the middleware surfaces between the ends, or an explicit `n/a: <reason>`. Two specs that share an agreement no entry records is the planning defect this catches. The registry is **advisory until the grading gate ships** (nothing hard-fails on its absence), but emitting it is a planner deliverable now.
...
5. **Record it** in `board.json`: the `tracks` array (each with only `id`, ordered `slices`, and `depends_on` — `board.json` is a pure plan; a track's worktree path and state are derived from refs, never written here, per invariant 5), validated against `board-v1`. The Tracks table and touchpoint matrix in `index.md` are rendered from it. Track ids follow `T<N>-<short-kebab-name>` (e.g. `T1-identity-account`).
...
- [ ] **Self-contained acceptance checks** — an implementer reading the ACs plus the spec's typed `references` can derive every implementation task. Vague ACs ("fix the bug") or intake-level restatements ("add ticker search") fail — only implementation-precision ACs pass ("the Ticker input renders <TickerSearch /> with accessToken prop", "the Name field has disabled={true}").
- [ ] **Typed normative references** — every contract registry entry, sibling slice spec, or workspace file whose contents are necessary to interpret an AC appears exactly once in `references` as `contract`, `slice`, or `file`. Do not rely on a path or identifier mentioned only in prose, `touchpoints`, or `test_refs`; those fields are not reference-discovery surfaces. Emit an empty array when no normative input is consumed.
...
1. For each track in `board.json` whose branch exists (`git rev-parse --verify track/<release-name>/<track-id>` succeeds — the materialisation signal, since worktree paths are derived not stored), read each of its slices' `status.json` from the **track branch** (`git show <track-branch>:docs/release/<release-name>/<slice>/status.json`).
...
- **Add a slice** → write its spec (Phase 4), then place it: a **new track**, or **appended to the end** of an existing track that is not `merged` and whose trailing slices have not started. The sole insertion exception is a mandatory maintainability rollback: place it immediately after its deferred failed slice and before the not-started tail. A new slice may **not** otherwise be inserted before a slice that is `in_progress`, `verified`, or `merged` — that breaks the track's sequential `start_commit` anchoring.
...
- **Drop or re-scope a started slice** → a human decision surfaced explicitly; `in_progress` / `verified` / `merged` work is never silently rewritten. When re-slicing resolves `maintainability.state: re_slice_required`, retain that terminal lifecycle and its append-only report ledger on the original slice id. Create a mandatory new rollback slice immediately after it (before any not-started tail), set the original `maintainability.rollback_slice_id`, and mark the original `deferred` with a Rule-2-complete record. The rollback contract restores the complete authored semantic envelope from the original `start_commit` through the rollback's own pinned head to the exact original mode/object ids; it therefore includes stray post-report production commits. It must verify before functional replacement slices may start and may never itself be deferred. Only then create replacement functionality as one or more later **new slices** with fresh template lifecycle records. `/merge-track` independently checks the rollback state and tree equality. Resetting the original id to cycle 0, or retaining failed bytes beneath a replacement base, is forbidden. A materially different spec for an already-`verified` slice is likewise a new slice, not an edit — verified work is immutable.
...
A re-plan runs on an in-flight release, so the release worktree already exists. Commit every planning artefact — new `spec.json` / `status.json`, `board.json`, `intake.md` — to the **release assembly branch `release-wt/<release-name>`**, working in the release worktree (conventional path `$HOME/projects/<REPO_BASENAME>-worktrees/release-<release-name>`, on branch `release-wt/<release-name>` — derived, not stored in `board.json`). Never commit re-plan work to the version integration branch (`release/v*` or `main`): that branch sits *above* `release-wt` in the track-mode hierarchy, and the release reaches it only via `/merge-release`, gated on every track verified. Committing to the integration branch directly jumps that gate and forces a backwards `integration → release-wt` sync to undo.
