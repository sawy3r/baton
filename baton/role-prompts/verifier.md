...
1. **Resolve ownership through the board oracle, never the launch-directory board or a persisted worktree field.** Run `sworn board --json` and find `<slice-id>` in `.releases["<release-name>"].tracks[]`, normalizing a null nested `slices` value to `[]`. Capture `<track-id>` and the ordered slice list. If absent, `BLOCKED: slice '<slice-id>' is not assigned to a track in board.json.` Worktree path/state are derived, not fields allowed by `board-v1`.
