...
1. **Resolve ownership through the board oracle, never the launch-directory board or a persisted worktree field.** Run `sworn board --json`, apply track-mode's projection integrity gate to the selected release, and require exactly one match for `<slice-id>` across normalized nested slice arrays with the row's `track` equal to its enclosing track id. Zero matches BLOCK as unassigned; duplicates, row/parent mismatch, or inconsistent normalized dependencies BLOCK as malformed oracle output. Capture the sole `<track-id>` and ordered slice list. Worktree path/state are derived, not fields allowed by `board-v1`.
...
3. Run `git worktree list --porcelain`; confirm the exact conventional path / `refs/heads/<worktree_branch>` stanza exists. If absent, `BLOCKED: track '<track-id>' worktree is missing on disk. Recreate with 'git worktree add <worktree_path> <worktree_branch>'.`
