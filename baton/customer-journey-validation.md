...
- **`assembled` is a derived state, not a stored one** (the same invariant that keeps `board.json` a pure plan): a release is `assembled` when `docs/release/<name>/assembly-proof.json` exists on `release-wt/<name>` with a passing verdict. No record stores the state; the artefact's existence and verdict are the state.
- **The assembly run** (reference implementation: `sworn assemble <release>`) brings up the stack from the release worktree, runs the release's deferred end-to-end set — no-mock, serially, with verified teardown — and emits `assembly-proof.json` (per-suite counts, preflight/API observations, screenshot paths). Non-zero exit on any non-excepted failure.
